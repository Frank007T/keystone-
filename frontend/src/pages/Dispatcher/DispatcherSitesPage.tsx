...
export function DispatcherSitesPage() {
      ...
      <div className="rounded-[32px] bg-white p-6 shadow-soft">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">Manage site locations</h2>
          </div>
        ...
        <div className="mt-6 overflow-hidden rounded-[24px] border border-slate-200">
          <table className="w-full border-collapse text-left text-sm text-slate-700">
            <thead className="bg-slate-50">
              <tr>
                {['Site Name', 'Customer', 'Address', 'Contact', 'Status', 'Actions'].map((heading) => (
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Loading sites...</td>
                </tr>
              ) : error ? (
                <tr>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-500">No sites found.</td>
                </tr>
              ) : (
                sites.map((site) => (
                  <tr key={site.name} className="border-t border-slate-200 hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">{site.contactPhone}</td>
