...
import { fetchDispatcherCustomers, User } from '../../lib/api';

export function DispatcherCustomersPage() {
  const [customers, setCustomers] = useState<User[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    // ✅ Changed from fetchManagerCustomers to fetchDispatcherCustomers
    fetchDispatcherCustomers()
      .then(setCustomers)
      ...
      <div className="rounded-[32px] bg-white p-6 shadow-soft">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">Manage customer relationships</h2>
          </div>
        ...
        <div className="mt-6 overflow-hidden rounded-[24px] border border-slate-200">
          <table className="w-full border-collapse text-left text-sm text-slate-700">
            <thead className="bg-slate-50">
              <tr>
                {['Company', 'Contact Person', 'Email', 'Phone', 'Status', 'Actions'].map((heading) => (
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Loading customers...</td>
                </tr>
              ) : error ? (
                <tr>
